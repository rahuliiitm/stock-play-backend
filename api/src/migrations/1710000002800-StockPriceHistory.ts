...
export class StockPriceHistory1710000002800 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    if (!tableExists) {
      ...
      `)
      await queryRunner.query(`CREATE UNIQUE INDEX ux_sph_symbol_asof ON stock_price_history(symbol, as_of);`)
    } else {
      // Table exists, check if it has the expected columns
      const columns = await queryRunner.getTable('stock_price_history')
      const hasAsOfColumn = columns?.columns.some(col => col.name === 'as_of')
      
      if (!hasAsOfColumn) {
        console.log('Stock price history table exists with different schema, skipping migration')
      }
    }
  }
