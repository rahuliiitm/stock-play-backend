...
export class NseSource implements MarketDataSource {
  ...
  async getQuote(symbol: string): Promise<Quote> {
    ...
    try {
      ...
      const last = Number(
        data?.lastPrice ?? data?.ltp ?? data?.price ?? data?.payload?.last_price ?? 0,
      )
      const ts = this.toIso(
  ...
  async getHistory(symbol: string, from?: string, to?: string, _intervalMinutes?: number): Promise<Candle[]> {
