...
@Injectable()
export class ATRTrailingStopComponent implements ITrailingStopComponent {
  private readonly logger = new Logger(ATRTrailingStopComponent.name);
  ...
  private checkTradeTrailingStop(
    ...
    // Apply maximum distance limit if configured
    ...
    if (trade.direction === 'LONG') {
      // For LONG positions, trail stop below highest price
      ...
      // For SHORT positions, trail stop above lowest price
  ...
  private updateTradeTrailingStop(
    ...
    this.logger.debug(
      `Updated trailing stop for ${trade.direction} trade ${trade.id}: ` +
