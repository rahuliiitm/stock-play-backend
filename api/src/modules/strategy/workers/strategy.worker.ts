...
import {
  ...
  CandleData,
  IndicatorValue,
} from '../services/strategy-building-blocks.service';
...
class StrategyWorker {
  ...
  private async checkExitConditions(): Promise<any | null> {
    ...
    const sortedConditions = [...config.exitConditions].sort(
   ...
   */
  private async evaluateEntryCondition(
    ...
    if (condition.type === 'SEQUENTIAL') {
      ...
      for (const strategyCondition of condition.conditions) {
        const satisfied =
          await this.evaluateStrategyCondition(strategyCondition);
        if (!satisfied) {
   ...
   */
  private async evaluateStrategyCondition(
   ...
   */
  private evaluateIndicatorComparison(
    condition: StrategyCondition,
    indicators: Record<string, IndicatorValue>,
  ): boolean {
    ...
    switch (operator) {
      case 'EQ':
        return indicatorValue === rightOperand;
      case 'NEQ':
        return indicatorValue !== rightOperand;
      case 'GT':
        return indicatorValue > rightOperand;
  ...
  private evaluatePriceCondition(
    ...
    switch (leftOperand) {
      case 'close':
        leftValue = marketData.close;
        break;
      case 'open':
        leftValue = marketData.open;
        break;
      case 'high':
        leftValue = marketData.high;
        break;
      case 'low':
        leftValue = marketData.low;
        break;
      case 'entry_candle_high':
        leftValue = this.state.entrySignal?.candle?.high;
        break;
      case 'entry_candle_low':
        leftValue = this.state.entrySignal?.candle?.low;
        break;
      default:
        return false;
    }
    ...
    const rightValue =
      typeof rightOperand === 'string' && rightOperand.includes('.')
  ...
  private evaluateTimeCondition(
    ...
    switch (leftOperand) {
      case 'day_of_week':
        leftValue = marketData.dayOfWeek;
        break;
      case 'hour':
        leftValue = marketData.hour;
        break;
      case 'days_to_expiry':
        leftValue = marketData.daysToExpiry;
        break;
      default:
        return false;
    }
  ...
  private async handleOrderExecuted(data: any): Promise<void> {
    ...
    console.log(
      `Order executed for strategy ${this.state.strategyId}:`,
      orderResult,
    );
  }
  ...
  private async handleExitExecuted(data: any): Promise<void> {
    ...
    console.log(
      `Exit executed for strategy ${this.state.strategyId}:`,
      exitResults,
    );
  }
