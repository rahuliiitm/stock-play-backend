...
export class StrategyManagementController {
  ...
  constructor(private strategyRunner: StrategyRunnerService) {}
      ...
      const strategies = [
        {
          ...
          status: runningStrategies.includes('nifty-weekly-option-selling')
            ? 'RUNNING'
            : 'STOPPED',
        },
      ];
  ...
  async getStrategyStatus(@Param('id') id: string) {
    try {
      ...
      throw new HttpException(
        'Failed to fetch strategy status',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async getStrategyStats(@Param('id') id: string) {
    try {
      ...
      throw new HttpException(
        'Failed to fetch strategy statistics',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async testSignal(@Param('id') id: string, @Body() body: any) {
    try {
      ...
      if (!candle || !indicators) {
        throw new HttpException(
          'Candle and indicators data required',
  ...
  async getRunningStrategies() {
    try {
      ...
      throw new HttpException(
        'Failed to fetch running strategies',
        HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
  ...
  async stopAllStrategies() {
    try {
      const runningStrategies = this.strategyRunner.getRunningStrategies();
      const stopPromises = runningStrategies.map((id) =>
        this.strategyRunner.stopStrategy(id),
      );
  ...
  async validateStrategyConfig(@Body() config: StrategyConfig) {
    try {
      ...
      if (!config.id || !config.name || !config.underlyingSymbol) {
        throw new HttpException(
          'Missing required strategy configuration fields',
          HttpStatus.BAD_REQUEST,
        );
      }
