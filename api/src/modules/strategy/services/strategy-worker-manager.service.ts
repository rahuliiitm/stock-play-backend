...
export class StrategyWorkerManager {
  ...
  async startStrategy(strategyId: string): Promise<void> {
    try {
      ...
      const worker = new Worker('./dist/modules/strategy/workers/strategy.worker.js', {
        workerData: {
          strategyId,
      ...
      this.logger.log(`✅ Started strategy ${strategyId} in worker thread ${worker.threadId}`)
  ...
  async restartStrategyWithState(
    ...
    try {
      ...
      this.logger.log(`🔄 Restarted strategy ${strategyId} with recovered state`)

    } catch (error) {
      this.logger.error(`Failed to restart strategy ${strategyId} with state:`, error)
      throw error
    }
  }
  ...
  private setupWorkerCommunication(strategyWorker: StrategyWorker): void {
    ...
    worker.on('exit', async (code) => {
      this.logger.warn(`Worker exited for strategy ${strategyId} with code ${code}`)
  ...
  private async handleWorkerMessage(strategyId: string, message: WorkerMessage): Promise<void> {
    ...
    switch (type) {
      ...
      default:
        this.logger.warn(`Unknown message type from strategy ${strategyId}: ${type}`)
    }
  }
  ...
  private async handleEntrySignal(strategyId: string, data: any): Promise<void> {
  ...
  private async handleAdjustmentSignal(strategyId: string, data: any): Promise<void> {
  ...
  private async handleExitSignal(strategyId: string, data: any): Promise<void> {
    ...
    await this.executionLogRepository.save(
      StrategyExecutionLog.positionClosed(strategyId, 'EXIT', data.position || {}, data.reason || 'SIGNAL')
    )
  }

  private async handleStateUpdate(strategyId: string, data: any): Promise<void> {
   ...
   * Wait for worker to stop gracefully
   */
  private async waitForWorkerStop(strategyWorker: StrategyWorker, timeoutMs: number): Promise<void> {
  ...
  private async checkWorkerHealth(): Promise<void> {
    ...
    for (const [strategyId, strategyWorker] of this.workers) {
      ...
      if (timeSinceHeartbeat > maxAllowedTime) {
        ...
        this.logger.warn(`Strategy ${strategyId} missed heartbeat (${timeSinceHeartbeat}ms)`)
      }
    }
    ...
    for (const strategyId of unhealthyWorkers) {
      try {
        ...
        this.logger.error(`Failed to restart unhealthy strategy ${strategyId}:`, error)
      }
    }
  }
