...
export class StrategyRecoveryService implements OnModuleInit {
  ...
  private async recoverRunningStrategies(): Promise<void> {
    try {
      ...
      this.logger.log(`✅ Strategy recovery completed: ${recoveryResults.successful} successful, ${recoveryResults.failed} failed, ${recoveryResults.skipped} skipped`)
   ...
   */
  private async recoverStrategy(strategyState: StrategyRuntimeState): Promise<boolean> {
    const { strategyId, currentPhase, phaseStates, lastProcessedCandle } = strategyState
    ...
    if (!strategy.isActive) {
      this.logger.warn(`Strategy ${strategyId} is not active, skipping recovery`)
    ...
    try {
      ...
      this.logger.error(`Failed to restart strategy worker for ${strategyId}:`, error)
      return false
    }
  }
  ...
  private async handleMissedData(strategy: Strategy, lastProcessedCandle: any): Promise<void> {
    ...
    // Calculate the gap
    const gapMinutes = (currentTime.getTime() - missedStartTime.getTime()) / (1000 * 60)
    ...
    this.logger.log(`📊 Detected ${gapMinutes.toFixed(1)} minute gap for strategy ${strategy.id}`)
    ...
    const missedData = await this.missedDataRepository.save({
      ...
      candlesCount: Math.floor(gapMinutes / this.timeframeToMinutes(strategy.timeframe))
    })

    // For large gaps, we could trigger background processing
    if (gapMinutes > 60) { // More than 1 hour gap
      this.logger.warn(`Large gap detected (${gapMinutes.toFixed(1)} min), consider manual data processing for strategy ${strategy.id}`)
  ...
  private async getHistoricalData(
    ...
    this.logger.debug(`Getting historical data for ${symbol} from ${startTime} to ${endTime}`)
    return []
  }
   ...
   */
  private async validatePositions(strategyId: string, positions: any[]): Promise<void> {
  ...
  async recoverSpecificStrategy(strategyId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(`Manual recovery failed for strategy ${strategyId}:`, error)
      return false
    }
  }
  ...
  async forceCleanup(maxAgeMinutes: number = 60): Promise<number> {
    try {
      const cleanedCount = await this.statePersistence.cleanupStaleStates(maxAgeMinutes)
