...
export class StrategyStatePersistenceService {
  ...
  async saveStrategyState(
    strategyId: string,
    state: Partial<StrategyRuntimeState>,
  ): Promise<void> {
    try {
      ...
      if (existingState) {
        await this.stateRepository.update({ strategyId }, stateData);
    ...
    try {
      ...
      if (redisState) {
        ...
        if (parsedState.lastProcessedCandle?.timestamp) {
          parsedState.lastProcessedCandle.timestamp = new Date(
            parsedState.lastProcessedCandle.timestamp,
          );
        }
        return parsedState;
      }
      ...
      if (dbState) {
        this.logger.debug(
          `Loaded state from database for strategy ${strategyId}`,
        );
        return dbState;
      }
      ...
      this.logger.error(
        `Failed to load state for strategy ${strategyId}:`,
        error,
      );
      return null;
    }
  ...
  async markStrategyAsStopped(strategyId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to mark strategy ${strategyId} as stopped:`,
        error,
      );
      throw error;
    }
  ...
  async updateHeartbeat(strategyId: string): Promise<void> {
    try {
      ...
      this.logger.error(
        `Failed to update heartbeat for strategy ${strategyId}:`,
        error,
      );
    }
  }
  ...
  async getHealthStats(): Promise<{
    ...
    try {
      ...
      const healthyStates = runningStates.filter(
        (s) => now - s.lastHeartbeat.getTime() < 5 * 60 * 1000, // 5 minutes
      );

      const cutoffTime = new Date(Date.now() - 30 * 60 * 1000);
      const staleStates = allStates.filter(
        (s) => s.isRunning && s.lastHeartbeat < cutoffTime,
      );
    ...
    try {
      ...
      for (const key of redisKeys) {
        ...
        if (redisState) {
          ...
          // Compare key fields
          if (
            redisData.isRunning === dbState.isRunning &&
