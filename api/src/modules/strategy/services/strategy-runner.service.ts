...
import { StrategyConfig, EntryCondition, ExitCondition } from '../interfaces/strategy-config.interface'
...
import { StrategyBuildingBlocksService, CandleData, IndicatorValue } from './strategy-building-blocks.service'
import { GrowwApiService } from '../../broker/services/groww-api.service'

interface StrategyWorkerMessage {
  type: 'INIT' | 'CANDLE_UPDATE' | 'SIGNAL_GENERATED' | 'ORDER_EXECUTED' | 'ERROR' | 'HEARTBEAT'
  strategyId: string
  data: any
  timestamp: Date
}
...
export class StrategyRunnerService implements OnModuleDestroy {
  ...
  async startStrategy(config: StrategyConfig): Promise<boolean> {
    try {
      ...
      const existingState = await this.statePersistence.loadStrategyState(config.id)
      ...
      const workerPath = path.resolve(__dirname, '../workers/strategy.worker.js')
      ...
      worker.on('exit', (code) => {
        this.logger.log(`Strategy ${config.id} worker exited with code ${code}`)
   ...
   */
  async processCandleData(strategyId: string, candle: CandleData, indicators: Record<string, IndicatorValue>): Promise<void> {
    const worker = this.workers.get(strategyId)
    if (!worker) {
      this.logger.warn(`Strategy ${strategyId} is not running`)
      return
    ...
    if (!context) {
      this.logger.error(`No execution context found for strategy ${strategyId}`)
      return
    }
   ...
   */
  private async handleWorkerMessage(message: StrategyWorkerMessage): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Error handling worker message for ${strategyId}:`, error)
    }
  }
   ...
   */
  private async handleSignalGenerated(strategyId: string, signalData: any): Promise<void> {
    const context = this.executionContexts.get(strategyId)
    if (!context) return
   ...
   */
  private async executeEntryOrder(strategyId: string, signalData: any): Promise<void> {
    try {
      const context = this.executionContexts.get(strategyId)
      if (!context) return
      ...
      if (!orderDetails) {
        this.logger.error(`Failed to generate order for strategy ${strategyId}`)
        return
      }
      ...
      if (orderResult && orderResult.orderId) {
        ...
        const position = await this.createPositionRecord(strategyId, orderResult, signalData)
        ...
        this.logger.error(`Order execution failed for strategy ${strategyId}:`, orderResult)
      ...
      this.logger.error(`Error executing entry order for ${strategyId}:`, error)
   ...
   */
  private async executeExitOrder(strategyId: string, signalData: any): Promise<void> {
    try {
      const context = this.executionContexts.get(strategyId)
      ...
      if (successfulOrders.length === exitOrders.length) {
        ...
        this.logger.error(`Partial exit failure for strategy ${strategyId}: ${failedOrders.length} failed orders`)
      }
   ...
   */
  private async handleOrderExecuted(strategyId: string, orderData: any): Promise<void> {
   ...
   */
  private async handleStrategyError(strategyId: string, error: any): Promise<void> {
   ...
   */
  private async handleHeartbeat(strategyId: string, heartbeatData: any): Promise<void> {
   ...
   */
  private generateOrderFromSignal(config: StrategyConfig, signalData: any): any {
   ...
   * Create position record
   */
  private async createPositionRecord(strategyId: string, orderResult: any, signalData: any): Promise<StrategyPosition> {
   ...
   * Log strategy event
   */
  private async logStrategyEvent(strategyId: string, action: string, details: any): Promise<void> {
    try {
