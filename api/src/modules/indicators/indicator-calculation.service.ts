...
export class IndicatorCalculationService {
  ...
  async calculateIndicator(
    ...
    try {
      ...
      const candles = await this.getHistoricalData(
        symbol,
        provider.minDataPoints,
      );
      if (!candles || candles.length < provider.minDataPoints) {
        this.logger.warn(
          `Insufficient data for ${symbol} ${indicatorName}: ${candles?.length || 0} candles`,
        );
        return null;
      }
      ...
      this.logger.log(
        `Calculated ${indicatorName} for ${symbol}: ${result.value}`,
      );

      return savedValue;
    } catch (error) {
      this.logger.error(
        `Error calculating ${indicatorName} for ${symbol}:`,
        error,
      );
      return null;
    }
   ...
   */
  async calculateAllIndicatorsForSymbol(
    symbol: string,
    ...
    for (const timeframe of timeframes) {
      try {
        ...
        if (!candles || candles.length === 0) {
          this.logger.warn(
            `No trading candle data for ${symbol} on ${timeframe}`,
          );
          continue;
        }

        // Get active indicator configs for this symbol and timeframe
        const configs = await this.getIndicatorConfigsForTimeframe(
          symbol,
          timeframe,
          indicatorNames,
        );
        ...
        for (const config of configs) {
          try {
            ...
            this.logger.debug(
              `Calculated ${config.indicator_name} for ${symbol} on ${timeframe}: ${result.value}`,
            );
          } catch (error) {
            this.logger.error(
              `Error calculating ${config.indicator_name} for ${symbol} on ${timeframe}:`,
              error,
            );
          }
        }
        ...
        this.logger.error(
          `Error processing timeframe ${timeframe} for ${symbol}:`,
          error,
        );
      }
    }
   ...
   * Get candle data from trading module's aggregated data
   */
  private async getTradingCandleData(
    symbol: string,
    ...
    try {
      ...
      this.logger.debug(
        `Getting trading candle data for ${symbol} on ${timeframe}`,
      );
      ...
      const startDate = new Date(
        endDate.getTime() - minDataPoints * intervalMinutes * 60 * 1000,
      );
      ...
      if (!history || history.length < 20) {
        this.logger.warn(
          `Insufficient trading data for ${symbol} on ${timeframe}: ${history?.length || 0} candles`,
        );
        return null;
      }
      ...
      this.logger.error(
        `Error getting trading candle data for ${symbol} on ${timeframe}:`,
        error,
      );
      return null;
    }
  ...
  private async getIndicatorConfigsForTimeframe(
    ...
    if (indicatorNames.length > 0) {
      query.andWhere('config.indicator_name IN (:...names)', {
  ...
  private async getHistoricalData(
    ...
    try {
      ...
      const endDate = new Date();
      const startDate = new Date(
        endDate.getTime() - minDataPoints * 24 * 60 * 60 * 1000,
      ); // minDataPoints days ago
      ...
      if (!history || history.length < minDataPoints) {
        this.logger.warn(
          `Insufficient historical data for ${symbol}: ${history?.length || 0} candles, need ${minDataPoints}`,
        );
        return null;
      }
