...
export class SupertrendProvider implements IndicatorProvider {
  name = 'SUPERTREND'
  description = 'Supertrend - Trend following indicator that combines ATR and price action to identify trend direction and potential reversal points'
  ...
  calculate(candles: Candle[], parameters: Record<string, any>): IndicatorResult | null {
    ...
    try {
      ...
      const trendDirection = currentPrice > latestSupertrend ? 'UPTREND' : 'DOWNTREND'

      // Calculate signal strength (distance from Supertrend)
      const signalStrength = Math.abs(currentPrice - latestSupertrend) / latestSupertrend * 100
      ...
      return {
        ...
        additionalData: {
          ...
          signal: this.generateSignal(currentPrice, latestSupertrend, trendDirection, signalStrength),
