...
export class PortfolioJobsService implements OnModuleInit {
  ...
  constructor(
    ...
    private readonly prices: Repository<StockPriceHistory>,
      ...
      async (job) => {
        if (job.name === 'create-snapshot') {
          await this.handleCreateSnapshot(
            job.data as { portfolioId: string; asOfISO: string },
          );
        }
      },
      { connection },
  ...
  async enqueueCreateSnapshot(portfolioId: string, asOfISO: string) {
    const opts: JobsOptions = {
      jobId: `create-snapshot:${portfolioId}:${asOfISO}`,
      attempts: 3,
  ...
  private async handleCreateSnapshot(data: {
    portfolioId: string;
    asOfISO: string;
  }) {
    ...
    for (const holding of holdings) {
      let priceRow = await this.prices.findOne({
        where: { symbol: holding.symbol, as_of: new Date(data.asOfISO) },
      });
      if (!priceRow) {
        priceRow = await this.prices
          .createQueryBuilder('h')
          .where('h.symbol = :sym AND h.as_of <= :asOf', {
            sym: holding.symbol,
            asOf,
          })
      ...
      if (priceRow) {
        ...
        const currentValueCents = Math.round(
          currentPriceCents * parseFloat(holding.quantity),
        );
        ...
        holding.current_value = Math.round(
          quote.price * parseFloat(holding.quantity),
        );
        await this.holdings.save(holding);
    ...
    const returnPercent =
      initialValueCents > 0
        ? ((totalValueCents - initialValueCents) / initialValueCents) * 100
        : 0;
