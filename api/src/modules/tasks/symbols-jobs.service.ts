...
export class SymbolsRefreshedEvent {
  constructor(
    public readonly exchange: 'NSE' | 'BSE',
...
function guessBseBhavcopyUrl(d: Date): string[] {
  const base =
    process.env.BSE_BHAVCOPY_BASE ||
    'https://www.bseindia.com/download/BhavCopy/Equity/';
  const { dd, mm, yy } = formatDateDDMMYY(d);
  return [
    `${base}EQ_${dd}${mm}${yy}_CSV.ZIP`,
    `${base}EQ${dd}${mm}${yy}_CSV.ZIP`,
  ];
}
...
export class SymbolsJobsService implements OnModuleInit {
      ...
      async (job) => {
        if (job.name === 'sync-nse') {
          ...
          await this.eventsQueue?.add(
            'symbols.refreshed',
            new SymbolsRefreshedEvent('NSE', job.data.dateISO),
          );
        }
        if (job.name === 'sync-bse-bhavcopy') {
          const date = job.data?.dateISO
            ? new Date(job.data.dateISO)
            : new Date();
          ...
          for (const url of urls) {
            try {
              ...
              await this.eventsQueue?.add(
                'symbols.refreshed',
                new SymbolsRefreshedEvent('BSE', job.data.dateISO),
              );
              return { ok: true, url };
            } catch (e) {
              lastErr = e;
  ...
  async enqueueDaily() {
    ...
    await this.symbolsQueue.add(
      'sync-nse',
      { dateISO: todayISO },
      { ...opts, jobId: `sync-nse:${todayISO}` },
    );
    await this.symbolsQueue.add(
      'sync-bse-bhavcopy',
      { dateISO: todayISO },
      { ...opts, jobId: `sync-bse-bhavcopy:${todayISO}` },
    );
  }
}
