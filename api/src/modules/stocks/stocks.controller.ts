...
export class StocksController {
  ...
  @Get('symbols')
  listSymbols(@Query('q') q?: string, @Query('exchange') exchange?: string) {
    return this.symbols.list(q, exchange);
  }

  @Get('symbols/:symbol')
  getSymbol(@Param('symbol') symbol: string) {
  ...
  syncSymbols(@Body() body: { nseUrl?: string; bseUrl?: string }) {
    return this.symbols.sync(body.nseUrl, body.bseUrl);
  }
  ...
  async indicatorsEndpoint(@Param('symbol') symbol: string) {
    ...
    return {
      rsi: this.indicators.rsi(candles),
      ma20: this.indicators.ma(candles, 20),
      macd: this.indicators.macd(candles),
    };
  }
}
