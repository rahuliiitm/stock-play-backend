...
export class SymbolsService {
  ...
  private defaultSources() {
    const nse = process.env.NSE_SYMBOLS_URL || 'https://archives.nseindia.com/content/equities/EQUITY_L.csv'
  ...
  async sync(nseUrl?: string, bseUrl?: string) {
    ...
    for (const s of sources) {
      try {
        if (!s.url) continue
        const { data } = await this.http.axiosRef.get(s.url, { responseType: 'text', headers: { 'User-Agent': 'Mozilla/5.0' } })
        ...
        for (let i = 1; i < rows.length; i++) {
          ...
          if (isNse) {
            ...
            await this.symbols
              ...
              .insert()
              .into(StockSymbol)
              .values({ symbol, name, exchange: s.exchange, series, isin, lot_size: lot ? Number(lot) : null, face_value: face ? face : null, status: 'active', last_seen_at: batchAt })
              .orUpdate(['name', 'series', 'isin', 'lot_size', 'face_value', 'status', 'last_seen_at', 'updated_at'], ['symbol', 'exchange'])
              .execute()
          } else if (isBse) {
            // BSE Bhavcopy CSV (direct CSV variant) columns e.g. TckrSymb, FinInstrmNm, SctySrs, ISIN
            const raw = (get('TCKRSYMB') || get('SYMBOL') || r[0] || '').toUpperCase()
            const symbol = raw.replace(/[\s#]+/g, '')
            if (!symbol || symbol === 'TCKRSYMB') continue
            const name = get('FININSTRMNM') || get('SC_NAME') || get('SCRIP NAME') || symbol
            ...
            await this.symbols
              ...
              .insert()
              .into(StockSymbol)
              .values({ symbol, name, exchange: s.exchange, series, isin, status: 'active', last_seen_at: batchAt })
              .orUpdate(['name', 'series', 'isin', 'status', 'last_seen_at', 'updated_at'], ['symbol', 'exchange'])
              .execute()
          }
        }
        ...
        await this.symbols
          ...
          .where('exchange = :ex AND (last_seen_at IS NULL OR last_seen_at < :batchAt)', { ex: s.exchange, batchAt })
          .execute()
      } catch (e: any) {
        this.logger.warn(`Symbol sync failed for ${s.exchange}: ${e?.message || e}`)
      }
    }
    return { ok: true }
  }
  ...
  async syncBseBhavcopy(zipUrl: string) {
    const batchAt = new Date()
    const { data } = await this.http.axiosRef.get(zipUrl, { responseType: 'arraybuffer', headers: { 'User-Agent': 'Mozilla/5.0' } })
    const AdmZip = (await import('adm-zip')).default
    const zip = new AdmZip(Buffer.from(data))
    const entries = zip.getEntries().filter((e: any) => e.entryName.toLowerCase().endsWith('.csv'))
    for (const e of entries) {
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        const symbol = (get('SC_NAME') || get('SCRIP NAME') || get('SYMBOL') || r[0] || '').toUpperCase().replace(/\s+/g, '')
        if (!symbol) continue
        ...
        await this.symbols
          ...
          .insert()
          .into(StockSymbol)
          .values({ symbol, name, exchange: 'BSE', isin, status: 'active', last_seen_at: batchAt })
          .orUpdate(['name', 'isin', 'status', 'last_seen_at', 'updated_at'], ['symbol'])
          .execute()
      }
      await this.symbols
        .createQueryBuilder()
        .update(StockSymbol)
        .set({ status: 'inactive' as any })
        .where('exchange = :ex AND (last_seen_at IS NULL OR last_seen_at < :batchAt)', { ex: 'BSE', batchAt })
        .execute()
    }
    return { ok: true }
  }
  ...
  async list(q?: string, exchange?: string) {
    const qb = this.symbols.createQueryBuilder('s').where('s.status = :status', { status: 'active' })
    if (q && q.trim().length > 0) qb.andWhere('(s.symbol ILIKE :q OR s.name ILIKE :q)', { q: `%${q}%` })
