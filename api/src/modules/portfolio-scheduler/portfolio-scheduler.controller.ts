...
export class PortfolioSchedulerController {
  ...
  async updateBrokerAccount(
    ...
    return this.brokerAccountsService.updateBrokerAccount(
      accountId,
      userId,
      body,
    );
  ...
  async getHoldings(
    ...
    try {
      ...
      if (
        holdingsResponse.data.status === 'SUCCESS' &&
        holdingsResponse.data.payload?.holdings
      ) {
  ...
  async getPositions(
    ...
    try {
      ...
      if (
        positionsResponse.data.status === 'SUCCESS' &&
        positionsResponse.data.payload?.positions
      ) {
  ...
  async getQuantityChanges(
    ...
    const [changes, total] =
      await this.orderQuantityChangesRepository.findAndCount({
        where: {
          symbol,
          order_history: {
    ...
    try {
      ...
      if (
        ...
        const currentValue = holdings.reduce(
          (sum, h) =>
            sum + (h.current_price ? h.quantity * h.current_price : 0),
          0,
        );
  ...
  async getPositionHistory(
    ...
    try {
      ...
      const positionHistory =
        await this.portfolioSyncSchedulerService.getPositionHistory(
          accountId,
          symbol,
        );
