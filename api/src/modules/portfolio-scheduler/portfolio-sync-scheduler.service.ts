...
import {
  OrderQuantityChange,
  ChangeType,
} from '../../entities/OrderQuantityChange.entity';
import { PortfolioSnapshot } from '../../entities/PortfolioSnapshot.entity';
import {
  SyncBatch,
  SyncType,
  SyncStatus,
} from '../../entities/SyncBatch.entity';
...
export class PortfolioSyncSchedulerService {
  ...
  async dailyPortfolioSync(): Promise<void> {
    if (this.isRunning) {
      this.logger.warn(
        'Portfolio sync already running, skipping this execution',
      );
      return;
    }
  ...
  private async syncAccountPortfolio(account: BrokerAccount): Promise<void> {
    ...
    try {
      this.logger.log(
        `📊 Syncing portfolio for account: ${account.id} (${account.broker})`,
      );
      ...
      const accessToken = await this.growwApiService.getAccessToken(
        account.api_key,
        account.api_secret,
      );
      ...
      if (orders.order_list && orders.order_list.length > 0) {
        this.logger.log(
          ...
          JSON.stringify(orders.order_list[0], null, 2),
        );
        this.logger.log(
          '📊 Available fields:',
          Object.keys(orders.order_list[0]),
        );
      }

      this.logger.log(
        `📈 Fetched data: ${holdings.length} holdings, ${positions.length} positions, ${orders.order_list?.length || 0} orders`,
      );
      ...
      await this.processOrderHistory(
        account,
        orders.order_list || [],
        syncBatch,
      );
      ...
      this.logger.error(
        `❌ Portfolio sync failed for account ${account.id}:`,
        error,
      );
  ...
  private async updateHoldings(
    ...
    this.logger.log(
      `📊 Updating ${holdings.length} holdings for account: ${account.id}`,
    );
  ...
  private async updatePositions(
    ...
    this.logger.log(
      `📊 Processing ${positions.length} positions for account: ${account.id} (APPEND-ONLY)`,
    );
    ...
    for (const position of positions) {
      ...
      if (!existingPosition) {
        ...
        positionsCreated++;
        this.logger.log(
          `📈 Created new position record: ${position.symbol} - Net Qty: ${position.net_quantity}`,
        );
      } else {
        this.logger.log(
          `📊 Position already exists: ${position.symbol} - Net Qty: ${position.net_quantity}`,
        );
      }
    }
    ...
    this.logger.log(
      `✅ Positions processed: ${positionsCreated} new records created (APPEND-ONLY)`,
    );
  }
  ...
  async getPositionHistory(accountId: string, symbol: string): Promise<any[]> {
    this.logger.log(
      `📊 Getting position history for ${symbol} in account: ${accountId}`,
    );
    ...
    this.logger.log(
      `📈 Found ${positionHistory.length} position records for ${symbol}`,
    );
  ...
  private async processOrderHistory(
    ...
    this.logger.log(
      `📊 Processing ${orders.length} orders for account: ${account.id}`,
    );
    ...
    for (const order of orders) {
      ...
      if (!existingOrder) {
        ...
        const changes = await this.checkOrderChanges(
          existingOrder,
          order,
          syncBatch,
        );
    ...
    this.logger.log(
      `📊 Order processing: ${ordersCreated} new orders, ${quantityChangesDetected} changes detected`,
    );
  }
  ...
  private async createOrderHistoryRecord(
    ...
    orderHistory.exchange_time = order.exchange_time
      ? new Date(order.exchange_time)
      : undefined;
    orderHistory.trade_date = order.trade_date
      ? new Date(order.trade_date)
      : undefined;
  ...
  private async checkOrderChanges(
    ...
    if (existingOrder.filled_quantity !== newOrderData.filled_quantity) {
      const quantityDelta =
        newOrderData.filled_quantity - existingOrder.filled_quantity;
  ...
  private async createQuantityChangeRecord(
    ...
    const safeQuantityDelta =
      quantityDelta ?? safeNewQuantity - safePreviousQuantity;
  ...
  private async createDailySnapshot(
    ...
    if (existingSnapshot) {
      this.logger.log(
        `📊 Snapshot already exists for today for account: ${account.id}`,
      );
      return;
    }
    ...
    const totalPositionsValue = positions.reduce(
      (sum, p) => sum + p.net_quantity * p.net_price,
      0,
    );
    ...
    const dayChangePercent =
      previousSnapshot && previousSnapshot.total_portfolio_value > 0
    ...
    this.logger.log(
      `📊 Created daily snapshot for account: ${account.id} - Value: ₹${totalPortfolioValue.toLocaleString()}`,
    );
  }
