...
export class BrokerAccountsService {
  ...
  async createBrokerAccount(
    ...
    this.logger.log(
      `Creating broker account for user: ${userId}, broker: ${brokerName}`,
    );
    ...
    try {
      ...
      this.logger.warn(
        `⚠️ Failed to validate API credentials: ${error.message}`,
      );
      this.logger.log(
        `Creating account without access token - will need to be validated later`,
      );
    }
    ...
    const savedAccount =
      await this.brokerAccountsRepository.save(brokerAccount);
    this.logger.log(
      `✅ Broker account created successfully: ${savedAccount.id}`,
    );
  ...
  async updateBrokerAccount(
    ...
    if (updates.apiKey && updates.apiSecret) {
      try {
        const accessToken = await GrowwApiService.getAccessToken(
          updates.apiKey,
          updates.apiSecret,
        );
        ...
        this.logger.log(
          `Updating credentials without validation - will need to be validated later`,
        );
    ...
    this.logger.log(
      `✅ Broker account updated successfully: ${updatedAccount.id}`,
    );
  ...
  async refreshAccessToken(
    ...
    const updatedAccount = await this.brokerAccountsRepository.save(account);
    this.logger.log(
      `✅ Access token refreshed successfully for account: ${accountId}`,
    );
