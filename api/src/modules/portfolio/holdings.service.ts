...
export class HoldingsService {
  ...
  async getPortfolioHoldings(portfolioId: string): Promise<HoldingSummary[]> {
    ...
    for (const holding of holdings) {
      ...
      if (cachedQuote) {
        ...
        const pnlPercent =
          invested > 0 ? Number(((pnl / invested) * 100).toFixed(4)) : 0;
        ...
        try {
          ...
          this.logger.error(
            `Failed to get quote for ${holding.symbol}:`,
            error,
          );
  ...
  async getHolding(
    ...
    const holding = await this.holdings.findOne({
      where: { portfolioId: portfolioId, symbol },
    });
  ...
  async updateHoldingCurrentValue(
    portfolioId: string,
    symbol: string,
  ...
  async getAllHoldingsWithSymbols(): Promise<
  ...
  async addHolding(
    ...
    this.logger.log(
      `Added holding ${symbol} to portfolio ${portfolioId}, updated quote cache`,
    );
    return savedHolding;
  }
  ...
  async removeHolding(portfolioId: string, symbol: string): Promise<void> {
    ...
    if (remainingHoldings.length === 0) {
      ...
      this.logger.log(
        `Removed holding ${symbol} from portfolio ${portfolioId}, cleared quote cache`,
      );
    } else {
      this.logger.log(
        `Removed holding ${symbol} from portfolio ${portfolioId}`,
      );
    }
  }
  ...
  async updateHoldingQuantity(
    ...
    this.logger.log(
      `Updated holding ${symbol} quantity in portfolio ${portfolioId}`,
    );
  }
}
