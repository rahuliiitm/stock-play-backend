...
export class PortfolioJobsService {
  ...
  async createDailyPortfolioSnapshots() {
    ...
    try {
      ...
      this.logger.log(
        `Processing ${allPortfolios.length} portfolios for daily snapshots`,
      );
      ...
      for (const portfolio of allPortfolios) {
        try {
          ...
          if (existingSnapshot) {
            this.logger.debug(
              `Snapshot already exists for portfolio ${portfolio.id} on ${today.toISOString()}`,
            );
            continue;
          }

          // Create new snapshot
          const snapshot = await this.portfolioService.createPortfolioSnapshot(
            portfolio.id,
            today,
          );
          ...
          if (snapshot) {
            ...
            this.logger.warn(
              `Failed to create snapshot for portfolio ${portfolio.id} (no holdings)`,
            );
          }
        } catch (error) {
          this.logger.error(
            `Error processing portfolio ${portfolio.id}:`,
            error,
          );
          errors++;
        }
      ...
      this.logger.log(
        `Daily portfolio snapshots completed: ${processed} processed, ${errors} errors`,
      );
    } catch (error) {
  ...
  async updatePortfolioCurrentValues() {
    ...
    try {
      ...
      this.logger.debug(
        `Updating current values for ${symbols.length} symbols`,
      );
      ...
      for (const symbol of symbols) {
        try {
          ...
          for (const holding of holdings) {
            ...
            await this.valuationService.invalidatePortfolioCache(
              holding.portfolioId,
            );
            updated++;
          }
          ...
          this.logger.error(
            `Error updating current values for ${symbol}:`,
            error,
          );
          errors++;
        }
      ...
      this.logger.debug(
        `Portfolio current values update completed: ${updated} updated, ${errors} errors`,
      );
    } catch (error) {
  ...
  async triggerDailySnapshots(): Promise<{
    processed: number;
    errors: number;
  }> {
  ...
  async triggerCurrentValuesUpdate(): Promise<{
    updated: number;
    errors: number;
  }> {
