...
export class ValuationService {
  ...
  async setCachedQuote(symbol: string, quote: CachedQuote): Promise<void> {
    ...
    try {
      await redis.setex(this.getQuoteCacheKey(symbol), this.QUOTE_CACHE_TTL, JSON.stringify(quote))
    } catch (error) {
      this.logger.error(`Error setting cached quote for ${symbol}:`, error)
    }
  }

  async getCachedPortfolioValuation(portfolioId: string): Promise<PortfolioValuation | null> {
    const redis = this.getRedisClient()
    if (!redis) return null
    ...
    try {
      ...
      this.logger.error(`Error getting cached portfolio valuation for ${portfolioId}:`, error)
      return null
    }
  }

  async setCachedPortfolioValuation(portfolioId: string, valuation: PortfolioValuation): Promise<void> {
    const redis = this.getRedisClient()
    if (!redis) return
    ...
    try {
      ...
      await redis.setex(this.getPortfolioCacheKey(portfolioId), this.PORTFOLIO_CACHE_TTL, JSON.stringify(cacheData))
    } catch (error) {
      this.logger.error(`Error setting cached portfolio valuation for ${portfolioId}:`, error)
    }
  }
  ...
  async calculatePortfolioValuation(portfolioId: string): Promise<PortfolioValuation | null> {
    // Try cache first
    ...
    const returnPercent = totalInvestedCents > 0 ? Number(((pnlCents / totalInvestedCents) * 100).toFixed(4)) : 0
  ...
  async invalidatePortfolioCache(portfolioId: string): Promise<void> {
    ...
    if (redis) {
      try {
        ...
        this.logger.error(`Error invalidating portfolio cache for ${portfolioId}:`, error)
      }
    }
  }
  ...
  async invalidateQuoteCache(symbol: string): Promise<void> {
    ...
    if (redis) {
      try {
        ...
        this.logger.error(`Error invalidating quote cache for ${symbol}:`, error)
      }
    }
  }
