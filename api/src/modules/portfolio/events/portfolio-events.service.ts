...
export class PortfolioEventsService {
  ...
  // Portfolio events
  async emitPortfolioCreated(
    event: Omit<PortfolioCreatedEvent, 'timestamp'>,
  ): Promise<void> {
    ...
    this.logger.debug(
      `Emitting portfolio.created event: ${JSON.stringify(fullEvent)}`,
    );
    await this.eventEmitter.emitAsync('portfolio.created', fullEvent);
  }

  async emitPortfolioUpdated(
    event: Omit<PortfolioUpdatedEvent, 'timestamp'>,
  ): Promise<void> {
    ...
    this.logger.debug(
      `Emitting portfolio.updated event: ${JSON.stringify(fullEvent)}`,
    );
    await this.eventEmitter.emitAsync('portfolio.updated', fullEvent);
  }

  // Holding events
  async emitHoldingAdded(
    event: Omit<HoldingAddedEvent, 'timestamp'>,
  ): Promise<void> {
    ...
    this.logger.debug(
      `Emitting portfolio.holding.added event: ${JSON.stringify(fullEvent)}`,
    );
    await this.eventEmitter.emitAsync('portfolio.holding.added', fullEvent);
  }

  async emitHoldingRemoved(
    event: Omit<HoldingRemovedEvent, 'timestamp'>,
  ): Promise<void> {
    ...
    this.logger.debug(
      `Emitting portfolio.holding.removed event: ${JSON.stringify(fullEvent)}`,
    );
    await this.eventEmitter.emitAsync('portfolio.holding.removed', fullEvent);
  }

  // Snapshot events
  async emitPortfolioSnapshotCreated(
    event: Omit<PortfolioSnapshotCreatedEvent, 'timestamp'>,
  ): Promise<void> {
    ...
    this.logger.debug(
      `Emitting portfolio.snapshot.created event: ${JSON.stringify(fullEvent)}`,
    );
    await this.eventEmitter.emitAsync('portfolio.snapshot.created', fullEvent);
  }

  // Broker events
  async emitBrokerAccountLinked(
    userId: string,
    ...
    this.logger.debug(
      `Emitting broker.account.linked event: ${JSON.stringify(event)}`,
    );
    await this.eventEmitter.emitAsync('broker.account.linked', event);
  }

  async emitBrokerAccountUnlinked(
    userId: string,
    brokerAccountId: string,
    broker: string,
  ): Promise<void> {
    ...
    this.logger.debug(
      `Emitting broker.account.unlinked event: ${JSON.stringify(event)}`,
    );
    await this.eventEmitter.emitAsync('broker.account.unlinked', event);
  }

  async emitBrokerSyncCompleted(
    userId: string,
    brokerAccountId: string,
    success: boolean,
    ...
    this.logger.debug(
      `Emitting broker.sync.completed event: ${JSON.stringify(event)}`,
    );
