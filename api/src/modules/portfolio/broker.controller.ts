...
export class BrokerController {
  ...
  async unlinkGrowwAccount(@Req() req: any, @Param('accountId') accountId: string) {
    try {
      const success = await this.brokerService.unlinkGrowwAccount(req.user.sub, accountId)

      if (!success) {
        throw new HttpException('Broker account not found', HttpStatus.NOT_FOUND)
      }
  ...
  async getUserBrokerAccounts(@Req() req: any) {
    try {
      const accounts = await this.brokerService.getUserBrokerAccounts(req.user.sub)
  ...
  async getBrokerAccount(@Req() req: any, @Param('accountId') accountId: string) {
    try {
  ...
  @UsePipes(new ValidationPipe({ transform: true }))
  async syncBrokerAccount(@Req() req: any, @Param('accountId') accountId: string) {
    try {
      ...
      if (account.broker === 'groww') {
        result = await this.brokerService.syncGrowwHoldings(req.user.sub, account.account_id)
      } else {
        throw new HttpException('Unsupported broker type', HttpStatus.BAD_REQUEST)
      }
  ...
  async getBrokerAccountStatus(@Req() req: any, @Param('accountId') accountId: string) {
    try {
      // First verify the account belongs to the user
      const account = await this.brokerService.getBrokerAccount(accountId)
  ...
  async getGrowwAuthUrl(@Req() req: any) {
    ...
    const redirectUri = process.env.GROWW_REDIRECT_URI || `${process.env.APP_URL}/auth/groww/callback`

    if (!clientId) {
      throw new HttpException('Groww OAuth not configured', HttpStatus.NOT_IMPLEMENTED)
    }
  ...
  @Post('groww/callback')
  async handleGrowwCallback(@Req() req: any, @Body() body: { code: string; state: string }) {
    // TODO: Implement OAuth callback handling
    // This would exchange the authorization code for access/refresh tokens
    // and link the broker account

    throw new HttpException('Groww OAuth callback not yet implemented', HttpStatus.NOT_IMPLEMENTED)
  }
  ...
  async getGrowwOrders(@Req() req: any, @Param('accountId') accountId: string) {
    try {
      const orders = await this.brokerService.getGrowwOrders(req.user.sub, accountId)
  ...
  async getGrowwMargin(@Req() req: any, @Param('accountId') accountId: string) {
    try {
      const margin = await this.brokerService.getGrowwMargin(req.user.sub, accountId)
  ...
  async searchGrowwInstruments(@Query('q') query: string) {
    ...
    try {
      const instruments = await this.brokerService.searchGrowwInstruments(query)
