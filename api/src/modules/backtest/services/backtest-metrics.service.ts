...
export class BacktestMetricsService {
  ...
  private calculateRiskMetrics(result: BacktestResult): RiskMetrics {
    ...
    return {
      ...
      calmarRatio: this.calculateCalmarRatio(result.totalReturnPercentage, result.maxDrawdown),
      recoveryFactor: this.calculateRecoveryFactor(result.totalReturn, result.maxDrawdown)
    }
  }
   ...
   */
  private calculatePerformanceMetrics(result: BacktestResult): PerformanceMetrics {
    const equityCurve = result.equityCurve
    const returns = this.calculateReturns(equityCurve)
  ...
  private calculateTradeAnalysis(result: BacktestResult): TradeAnalysis {
    ...
    return {
      ...
      avgTradeDuration: durations.length > 0 ? durations.reduce((sum, d) => sum + d, 0) / durations.length : 0,
