...
export class BacktestDataService {
   ...
   */
  async prepareBacktestData(symbol: string, timeframe: string, startDate: Date, endDate: Date): Promise<any[]> {
    try {
      this.logger.log(`Preparing data for ${symbol} ${timeframe} from ${startDate} to ${endDate}`)
      ...
      this.logger.log(`Data preparation completed: ${cleanedData.length} candles`)
      return cleanedData
   ...
   * Validate data quality
   */
  private validateDataQuality(data: any[], symbol: string, timeframe: string): void {
    if (data.length === 0) {
    ...
    if (anomalies.length > 0) {
      this.logger.warn(`Found ${anomalies.length} price anomalies for ${symbol}`)
    }

    // Check for sufficient data
    if (data.length < 100) {
      this.logger.warn(`Limited data available: ${data.length} candles for ${symbol}`)
    }
  }
   ...
   * Find gaps in data
   */
  private findDataGaps(data: any[]): Array<{start: number, end: number, duration: number}> {
   ...
   */
  private findPriceAnomalies(data: any[]): Array<{timestamp: number, priceChange: number, prevClose: number, currClose: number}> {
    const anomalies: Array<{timestamp: number, priceChange: number, prevClose: number, currClose: number}> = []
