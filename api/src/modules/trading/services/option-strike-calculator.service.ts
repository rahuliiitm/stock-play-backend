...
export class OptionStrikeCalculatorService {
  ...
  async calculateStrikes(
    ...
    const atmStrike = this.calculateATMStrike(
      ltp,
      config.strikeIncrement || 50,
    );
    const strikes: OptionStrike[] = [];

    // Process call strikes
    for (const strikeType of config.callStrikes) {
      const strike = this.getStrikeForType(
        strikeType,
        atmStrike,
        config.strikeIncrement || 50,
      );
      const symbol = this.generateOptionSymbol(
        underlying,
        expiry,
        strike,
        'CE',
