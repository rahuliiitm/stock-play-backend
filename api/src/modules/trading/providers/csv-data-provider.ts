...
export class CsvDataProvider implements MarketDataProvider {
  ...
  constructor() {
    this.dataDirectory = process.env.CSV_DATA_DIR || path.join(process.cwd(), 'data')
    this.logger.log(`CsvDataProvider initialized with data directory: ${this.dataDirectory}`)
  ...
  async getHistoricalCandles(
    ...
    try {
      this.logger.log(`Loading historical candles for ${symbol} ${timeframe} from ${startDate.toISOString()} to ${endDate.toISOString()}`)
      this.logger.log(`Data directory: ${this.dataDirectory}`)
      const filePath = path.join(this.dataDirectory, `${symbol}_${timeframe}.csv`)
      ...
      return new Promise((resolve, reject) => {
        ...
        fs.createReadStream(filePath)
          ...
          .on('end', () => {
            ...
            this.logger.log(`Loaded ${sortedCandles.length} candles for ${symbol} ${timeframe}`)
  ...
  async getOptionChain(symbol: string, expiry?: string): Promise<OptionChainData | null> {
    try {
      const filePath = path.join(this.dataDirectory, `option_chain_${symbol}.csv`)
      ...
      return new Promise((resolve, reject) => {
        fs.createReadStream(filePath)
          ...
          .on('error', (error) => {
            this.logger.error(`Error reading option chain file ${filePath}:`, error)
            reject(error)
          })
      })
    } catch (error) {
  ...
  async loadSymbolData(symbol: string, timeframes: string[]): Promise<void> {
    for (const timeframe of timeframes) {
      try {
        const filePath = path.join(this.dataDirectory, `${symbol}_${timeframe}.csv`)
        if (fs.existsSync(filePath)) {
          await this.getHistoricalCandles(symbol, timeframe, new Date(0), new Date())
        }
      } catch (error) {
        this.logger.warn(`Failed to load data for ${symbol} ${timeframe}:`, error)
      }
    }
  }
  ...
  async prepareNIFTYData(): Promise<void> {
    try {
      ...
      if (fs.existsSync(processedFilePath)) {
        this.logger.log('Processed NIFTY 15m data already exists, skipping aggregation')
        return
      }
      ...
      await new Promise<void>((resolve, reject) => {
        fs.createReadStream(rawFilePath)
          ...
          .on('end', () => {
            this.logger.log(`Loaded ${rawData.length} 1-minute candles from last year`)
            resolve()
          })
          .on('error', (error) => {
      ...
      this.logger.log(`Aggregated into ${aggregatedCandles.length} 15-minute candles`)

      // Write 15-minute data to CSV
      const csvHeaders = 'timestamp,open,high,low,close,volume\n'
      const csvData = aggregatedCandles.map(candle =>
  ...
  async getNIFTYLastYearData(): Promise<CandleData[]> {
    try {
      ...
      return await this.getHistoricalCandles('NIFTY', '15m', oneYearAgo, new Date())
    } catch (error) {
