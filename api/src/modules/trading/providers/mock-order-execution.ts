...
export class MockOrderExecutionProvider implements OrderExecutionProvider {
  ...
  async placeBuyOrder(order: OrderRequest): Promise<OrderResult> {
    this.logger.log(
      `Mock BUY order: ${order.quantity} ${order.symbol} @ ${order.price || 'MARKET'}`,
    );
    ...
    this.updatePosition(
      order.symbol,
      order.quantity,
      orderStatus.averagePrice || order.price || 0,
      'BUY',
    );
  ...
  async placeSellOrder(order: OrderRequest): Promise<OrderResult> {
    this.logger.log(
      `Mock SELL order: ${order.quantity} ${order.symbol} @ ${order.price || 'MARKET'}`,
    );
    ...
    if (existingPosition) {
      ...
      if (newQuantity === 0) {
        ...
        this.positions.set(symbol, {
          ...
          pnlPercentage:
            ((this.getMockPrice(symbol) - newAveragePrice) / newAveragePrice) *
            100,
        });
      }
    } else if (quantity > 0) {
