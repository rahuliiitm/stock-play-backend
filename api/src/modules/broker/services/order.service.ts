...
import { GrowwApiService, GrowwOrder, GrowwOrderResponse } from './groww-api.service'
...
export class OrderService {
  ...
  async placeOrder(request: OrderPlacementRequest): Promise<StrategyOrder> {
    const { strategyId, symbol, side, quantity, orderType, productType } = request

    this.logger.log(`📋 Placing ${side} order for ${symbol} (Strategy: ${strategyId})`)
    ...
    try {
      ...
      const growwResponse = await this.placeOrderWithRetry(growwOrder, savedOrder.id)
      ...
      const finalOrder = await this.orderRepository.findOne({ where: { id: savedOrder.id } })
      if (!finalOrder) {
   ...
   */
  async modifyOrder(orderId: string, modifications: OrderModificationRequest): Promise<StrategyOrder> {
    try {
      ...
      if (modifications.price !== undefined) modifyPayload.price = modifications.price
      if (modifications.quantity !== undefined) modifyPayload.quantity = modifications.quantity
      if (modifications.triggerPrice !== undefined) modifyPayload.triggerPrice = modifications.triggerPrice

      // Modify order with Groww API
      const growwResponse = await this.growwApiService.modifyOrder(order.brokerOrderId, modifyPayload)
      ...
      if (modifications.quantity !== undefined) updates.quantity = modifications.quantity
      if (modifications.triggerPrice !== undefined) updates.triggerPrice = modifications.triggerPrice
      ...
      const modifiedOrder = await this.orderRepository.findOne({ where: { id: orderId } })
      if (!modifiedOrder) {
  ...
  async cancelOrder(orderId: string): Promise<StrategyOrder> {
    try {
      ...
      const growwResponse = await this.growwApiService.cancelOrder(order.brokerOrderId)
      ...
      const cancelledOrder = await this.orderRepository.findOne({ where: { id: orderId } })
      if (!cancelledOrder) {
   ...
   */
  async getOrdersForStrategy(strategyId: string, status?: string): Promise<StrategyOrder[]> {
  ...
  async syncOrderStatus(orderId: string): Promise<StrategyOrder> {
    try {
      ...
      const brokerStatus = await this.growwApiService.getOrderStatus(order.brokerOrderId)

      // Update order based on broker status
      const statusUpdate = this.mapBrokerStatusToOrderStatus(brokerStatus.status)
      ...
      const syncedOrder = await this.orderRepository.findOne({ where: { id: orderId } })
      if (!syncedOrder) {
      ...
      this.logger.error(`❌ Failed to sync order ${orderId} status:`, error.message)
      throw error
    }
  }
  ...
  async syncAllOrdersForStrategy(strategyId: string): Promise<StrategyOrder[]> {
    const pendingOrders = await this.getOrdersForStrategy(strategyId, 'PENDING')
  ...
  private validateOrderRequest(request: OrderPlacementRequest): void {
    ...
    if (orderType === 'ICEBERG' && (!request.disclosedQuantity || request.disclosedQuantity >= quantity)) {
      throw new Error('Iceberg orders require disclosed quantity less than total quantity')
    }
  }
   ...
   * Place order with retry logic
   */
  private async placeOrderWithRetry(growwOrder: GrowwOrder, orderId: string): Promise<GrowwOrderResponse> {
    ...
    for (let attempt = 1; attempt <= this.MAX_RETRIES; attempt++) {
      try {
        ...
        if (attempt > 1) {
          await this.orderRepository.update(orderId, { retryCount: attempt - 1 })
        }
        ...
        if (attempt < this.MAX_RETRIES) {
          this.logger.warn(`Order placement attempt ${attempt} failed, retrying in ${this.RETRY_DELAY}ms...`)
          await new Promise(resolve => setTimeout(resolve, this.RETRY_DELAY))
        }
      }
    }

    throw new Error(`Order placement failed after ${this.MAX_RETRIES} attempts: ${lastError?.message || 'Unknown error'}`)
  }

  /**
   * Update order with broker response
   */
  private async updateOrderWithBrokerResponse(orderId: string, growwResponse: GrowwOrderResponse): Promise<void> {
    await this.orderRepository.update(orderId, {
   ...
   */
  private mapBrokerStatusToOrderStatus(brokerStatus: string): { status: string, executed: boolean } {
   ...
   */
  private async updatePositionOnExecution(order: StrategyOrder, brokerStatus: any): Promise<void> {
    if (!order.positionId) return

    const position = await this.positionRepository.findOne({ where: { id: order.positionId } })
    if (!position) return
  ...
  async getOrderStatistics(strategyId: string): Promise<{
    ...
    const stats = {
      ...
      executedOrders: orders.filter(o => o.status === 'EXECUTED' || o.status === 'PARTIAL_FILL').length,
      cancelledOrders: orders.filter(o => o.status === 'CANCELLED').length,
      rejectedOrders: orders.filter(o => o.status === 'REJECTED').length,
      successRate: 0
    }

    const completedOrders = stats.executedOrders + stats.cancelledOrders + stats.rejectedOrders
    stats.successRate = completedOrders > 0 ? (stats.executedOrders / completedOrders) * 100 : 0
