...
export class GrowwApiService {
  ...
  private async authenticateWithApiKey(
    ...
    try {
      ...
      this.logger.error(
        '❌ Groww API key authentication failed:',
        error.message,
      );
      return false;
    }
  ...
  private async authenticateWithCredentials(
    ...
    try {
      ...
      const response: AxiosResponse<GrowwAuthResponse> =
        await this.httpClient.post('/user/login', loginPayload);
      ...
      this.logger.error(
        '❌ Groww username/password authentication failed:',
        error.message,
      );
      return false;
    }
  ...
  async modifyOrder(
    orderId: string,
    modifications: Partial<GrowwOrder>,
    ...
    try {
      ...
      this.logger.log(
        `✅ Order modified successfully: ${orderResponse.orderId}`,
      );
      return orderResponse;
    } catch (error) {
  ...
  async cancelOrder(orderId: string): Promise<GrowwOrderResponse> {
    ...
    try {
      ...
      this.logger.log(
        `✅ Order cancelled successfully: ${orderResponse.orderId}`,
      );
      return orderResponse;
    } catch (error) {
  ...
  async getOrderList(page: number = 0, pageSize: number = 50): Promise<any> {
    ...
    try {
      ...
      this.logger.log(
        `✅ Order list retrieved - ${response.data.payload?.orders?.length || 0} orders`,
      );
      return response.data.payload;
    } catch (error) {
  ...
  async getOrderTrades(
    ...
    try {
      ...
      this.logger.log(
        `✅ Trades retrieved for order ${orderId} - ${response.data.payload?.trade_list?.length || 0} trades`,
      );
      return response.data.payload;
    } catch (error) {
      this.logger.error(
        `❌ Failed to get trades for order ${orderId}:`,
        error.message,
      );
  ...
  async getPositionForSymbol(
    ...
    try {
      ...
      this.logger.error(
        `❌ Failed to get position for ${tradingSymbol}:`,
        error.message,
      );
  ...
  async calculateRequiredMargin(
    ...
    try {
      this.logger.log(
        `💰 Calculating required margin for ${orders.length} orders`,
      );

      const response = await this.httpClient.post(
        `/v1/margins/detail/orders?segment=${segment}`,
        orders,
      );
  ...
  async getHistoricalData(
    ...
    try {
      this.logger.log(
        `📊 Getting historical data for ${symbol} from ${fromDate} to ${toDate}`,
      );
      ...
      this.logger.log(
        `✅ Historical data received for ${symbol}: ${candles.length} candles`,
      );
      return candles;
    } catch (error) {
      this.logger.error(
  ...
  private getCredentialsFromEnv(): GrowwCredentials | null {
    ...
    if (
      apiKey &&
      apiSecret &&
      apiKey !== 'your_actual_api_key_from_groww_execute_page'
    ) {
      return {
        apiKey,
        apiSecret,
  ...
  private async generateTOTP(secret: string): Promise<string> {
    try {
      ...
      this.logger.error('Failed to generate TOTP:', e?.message);
      throw e;
    }
  }
  ...
  private async generateChecksum(
    ...
    try {
      ...
      this.logger.error('Failed to generate checksum:', e?.message);
      throw e;
    }
  }
  ...
  private formatOrderPayload(order: Partial<GrowwOrder>): any {
    return {
      ...
      ...(order.orderValidityDate && {
        order_validity_date: order.orderValidityDate,
      }),
    };
  }
