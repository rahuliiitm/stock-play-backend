...
import type { OrderPlacementRequest, OrderModificationRequest } from '../services/order.service'
...
export class BrokerController {
   ...
   */
  @Post('auth')
  async authenticate(@Body() credentials: GrowwCredentials): Promise<{ success: boolean; message: string }> {
    try {
  ...
  async testConnectivity(): Promise<{ connected: boolean; message: string }> {
    try {
      ...
      return {
        connected,
        message: connected ? 'Broker connection successful' : 'Broker connection failed'
      }
    } catch (error) {
      return {
        connected: false,
  ...
  async getHistoricalData(
    ...
    try {
      ...
      const data = await this.growwApiService.getHistoricalData(symbol, fromDate, toDate, interval)
      return {
        symbol,
        interval,
        data,
  ...
  async getOrdersForStrategy(
    ...
    try {
      const orders = await this.orderService.getOrdersForStrategy(strategyId, status)
      return {
        strategyId,
        orders,
        count: orders.length,
  ...
  async syncAllOrdersForStrategy(@Param('strategyId') strategyId: string): Promise<any> {
    try {
      const orders = await this.orderService.syncAllOrdersForStrategy(strategyId)
      return {
        success: true,
        strategyId,
  ...
  async getOrderStatistics(@Param('strategyId') strategyId: string): Promise<any> {
    try {
  ...
  async getPendingOrdersCount(@Param('strategyId') strategyId: string): Promise<any> {
    try {
