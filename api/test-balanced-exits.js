...
async function runBalancedExitsTest() {
  console.log('🚀 Testing Balanced RSI Exits (65/35) Strategy');
  console.log('================================================');
  ...
  try {
    ...
    const balancedConfig = {
      ...
      strategyConfig: {
        ...
        rsiEntryLong: 50,
        rsiEntryShort: 50,
        rsiExitLong: 65,  // Less aggressive than 60
        rsiExitShort: 35, // Less aggressive than 40
        atrPeriod: 14,
    ...
    console.log(`   RSI Entry: ${balancedConfig.strategyConfig.rsiEntryLong}/${balancedConfig.strategyConfig.rsiEntryShort}`);
    console.log(`   RSI Exit: ${balancedConfig.strategyConfig.rsiExitLong}/${balancedConfig.strategyConfig.rsiExitShort}`);
    console.log(`   Supertrend: ${balancedConfig.strategyConfig.supertrendPeriod}, ${balancedConfig.strategyConfig.supertrendMultiplier}`);
    console.log('');

    const results = await backtestOrchestrator.runBacktest(balancedConfig);
    ...
    if (losingPercentage > 40) {
      console.log('   1. Try even more aggressive RSI exits (70/30)');
      console.log('   2. Add stop-loss protection');
    }
    if (winRate < 30) {
      console.log('   3. Consider less aggressive RSI exits (70/30)');
    ...
    console.error('❌ Error running balanced exits test:', error.message);
    console.error('Stack trace:', error.stack);
  }
}

runBalancedExitsTest();
