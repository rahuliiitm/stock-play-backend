 ...
 * Modifies the existing run-optimized-backtest-direct.js to test various trailing stop settings
 */
...
async function testTrailingStopConfigurations() {
  console.log('🚀 Testing Trailing Stop Configurations...\n');
  ...
  for (let i = 0; i < testConfigs.length; i++) {
    ...
    try {
      // Create a modified version of run-optimized-backtest-direct.js
...
async function runBacktestWithConfig(testConfig) {
  return new Promise((resolve, reject) => {
    // Create a temporary script file with the specific configuration
    const scriptContent = createBacktestScript(testConfig);
    const scriptPath = path.join(__dirname, `temp-backtest-${Date.now()}.js`);
...
function createBacktestScript(testConfig) {
  return `
