...
describe('Strategy Groww Integration (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    strategyRunner = moduleFixture.get<StrategyRunnerService>(
  ...
  describe('Complete Strategy Execution with Groww API', () => {
    it('should execute complete strategy lifecycle with real Groww API', async () => {
      console.log(
        '🚀 Starting complete strategy lifecycle test with Groww API',
      );
      ...
      await strategyRunner.processCandleData(
        config.id,
        mockCandleData,
        mockIndicators,
      );
    ...
    it('should handle option chain data retrieval and strike selection', async () => {
      ...
      if (optionChain.success) {
        ...
        const selectedStrikes = selectStrikesForSpread(
          spotPrice,
          optionChain.data.strikes,
          'BULL_PUT_SPREAD',
        );

        expect(selectedStrikes.sellStrike).toBeDefined();
        expect(selectedStrikes.buyStrike).toBeDefined();
        expect(selectedStrikes.sellStrike).toBeGreaterThan(
          selectedStrikes.buyStrike,
        );
        console.log('✅ Strike selection logic verified');
      } else {
        console.log(
          '⚠️ Option chain not available, skipping strike selection test',
        );
      }
    ...
    it('should handle order placement and management through Groww API', async () => {
      ...
      if (quote.success) {
        ...
        console.log(
          '📝 Order structure validated (real order placement commented for safety)',
        );
      } else {
    ...
    it('should handle market data streaming and indicator calculation', async () => {
      ...
      const indicators = await liveDataFeed.getIndicators('NIFTY', [
        'supertrend',
        'ema20',
      ]);
    ...
    it('should validate strategy performance metrics', async () => {
      ...
      const sharpeRatio = calculateSharpeRatio(
        testStrategy.totalPnL,
        testStrategy.maxDrawdown,
      );
  ...
  // Helper functions
  function selectStrikesForSpread(
    spotPrice: number,
    availableStrikes: any[],
    spreadType: string,
  ) {
