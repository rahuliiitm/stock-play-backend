...
import { NiftyOptionSellingService, NiftyOptionStrategy, SpreadPosition } from '../src/modules/strategy/services/nifty-option-selling.service'
import { StrategyBuildingBlocksService, CandleData, IndicatorValue } from '../src/modules/strategy/services/strategy-building-blocks.service'
...
describe('NiftyOptionSellingService', () => {
  ...
  describe('Entry Conditions Testing', () => {
    ...
    it('should not trigger entry signal when price is below EMA20 in bullish context', async () => {
      ...
      const result = await service.evaluateStrategy(strategy, invalidBullishContext)
    ...
    it('should not trigger entry signal when price is above EMA20 in bearish context', async () => {
      ...
      const result = await service.evaluateStrategy(strategy, invalidBearishContext)
  ...
  describe('Exit Conditions Testing', () => {
    ...
    it('should trigger exit signal near expiry (1 day remaining)', async () => {
      ...
      mockStrategyBlocks.evaluateCondition.mockImplementation((condition) => {
        if (condition.leftOperand === 'days_to_expiry' && condition.rightOperand === 1) {
          return Promise.resolve(true)
        }
        return Promise.resolve(false)
      })
  ...
  describe('Strike Selection and Order Generation', () => {
    ...
    it('should select appropriate strikes for bull put spread', async () => {
      ...
      const strikes = (service as any).selectStrikes(spotPrice, strategy, signalType)
    ...
    it('should generate correct spread order structure', () => {
      ...
      const order = (service as any).createSpreadOrder(strikes, signalType, strategy)
  ...
  describe('Time-based Logic Testing', () => {
    ...
    it('should trigger gamma risk exit on correct day', async () => {
      ...
      const wednesdayContext = {
        candle: { timestamp: Date.now(), open: 22000, high: 22100, low: 21900, close: 22050, volume: 1000000 },
        indicators: { supertrend_direction: { value: 'bullish', timestamp: new Date() } },
        marketData: { day: 3 },
        previousSignals: [],
        currentPositions: [position]
      }
      ...
      mockStrategyBlocks.evaluateCondition.mockImplementation((condition) => {
        return Promise.resolve(condition.leftOperand === 'day' && condition.rightOperand === 3)
      })
  ...
  describe('Risk Management Testing', () => {
    ...
    it('should trigger stop loss when loss exceeds threshold', () => {
      ...
      const result = (service as any).checkRiskManagementExit(strategy, stopLossPosition, {})
    ...
    it('should not trigger stop loss when loss is within threshold', () => {
      ...
      const result = (service as any).checkRiskManagementExit(strategy, withinThresholdPosition, {})
    ...
    it('should trigger partial profit when target is reached', () => {
      ...
      const result = (service as any).checkRiskManagementExit(strategy, profitPosition, {})
    ...
    it('should not trigger partial profit when below target', () => {
      ...
      const result = (service as any).checkRiskManagementExit(strategy, belowTargetPosition, {})
  ...
  describe('Edge Cases and Error Handling', () => {
    ...
    it('should handle network failures during option chain fetch', async () => {
      jest.spyOn(service as any, 'getOptionChain').mockRejectedValue(new Error('Network failure'))
  ...
  describe('Integration Scenarios', () => {
    ...
    it('should handle expiry-based exit logic', async () => {
      ...
      mockStrategyBlocks.evaluateCondition.mockImplementation((condition) => {
        return Promise.resolve(condition.leftOperand === 'days_to_expiry' && condition.rightOperand === 1)
      })
